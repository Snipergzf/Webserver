...
function _M.response(self)
	...
	local _tb = {action="commit_friend"}
	while true do
		...
		local result = self:commit_friend()
...
local function commit_friend(self)
	...
	if not res or not res[1] then 
		return const.ERR_API_COMMIT_FRIEND_FAILED_REQUIRE_NOT_EXISTS, nil
	end
...
_M.commit_friend = commit_friend
