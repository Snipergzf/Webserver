...
function _M.new(_, arg)
	...
	self.update_item = arg.update_item
	return self
end

function _M.response(self)
	local _, _em = common.try_load_model('group_action')
	local _tb = {action="update_group_info"}
	while true do
...
local function update(self)
	...
	res,err,errno,sqlstate = 
		db:query("UPDATE groups SET group_name = "..ngx.quote_sql_str(self.update_item).." WHERE group_id = "..ngx.quote_sql_str(self.group_id),10)
	if errno ~= nil and errno > 0 then
		return const.ERR_API_UPDATE_GROUP
	end
