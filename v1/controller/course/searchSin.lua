...
function _M.response(self)
	...
	local _tb = {action="searchByTeaName"}
	while true do
...
local function get_course(self)
	...
	local cursor = col:find({t_name = {["$regex"] = ngx.unescape_uri(self.search_name),["$options"] = 'i'}},{c_name=1,t_name=1,c_detail=1})
	for index, item in cursor:pairs() do
		table.insert (tb,index,item)
	end
	
	if tb[1] == nil then
		cursor = col:find({c_name = {["$regex"] = ngx.unescape_uri(self.search_name),["$options"]='i'}},{c_name=1,t_name=1,c_detail=1})
		for index, item in cursor:pairs() do
			table.insert (tb,index,item)
		end
	end
