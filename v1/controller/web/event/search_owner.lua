...
function _M.new(_, arg)
	...
	self.cEvent_owner = arg.cEvent_owner
	return self
end
...
local function search(self)
	...
	-- local r = col:find_one({cEvent_owner = self.cEvent_owner},{_id=1,cEvent_name=1,cEvent_time=1,cEvent_content=1,
	...
	local cursor = col:find({cEvent_owner = self.cEvent_owner},{_id=1,cEvent_name=1,cEvent_time=1,cEvent_content=1,
		cEvent_theme=1,cEvent_place=1,cEvent_provider=1,cEvent_publish=1,share_num=1,click_num=1,participate_num=1,cEvent_owner=1})
	for index, item in cursor:pairs() do
			table.insert(tb,index,item)
	end
	local col = db:get_col("cEvent_tmp")
	-- local r_ = col:find_one({cEvent_owner = self.cEvent_owner},{_id=1,cEvent_name=1,cEvent_time=1,cEvent_content=1,
		-- cEvent_theme=1,cEvent_place=1,cEvent_provider=1,cEvent_publish=1,share_num=1,click_num=1,participate_num=1,cEvent_owner=1})
	local cursor = col:find({cEvent_owner = self.cEvent_owner},{_id=1,cEvent_name=1,cEvent_time=1,cEvent_content=1,
		cEvent_theme=1,cEvent_place=1,cEvent_provider=1,cEvent_publish=1,share_num=1,click_num=1,participate_num=1,cEvent_owner=1})
	for index, item in cursor:pairs() do
